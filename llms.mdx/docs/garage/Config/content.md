...
### Lighting, Audio & Traffic [#lighting-audio--traffic]
...
| Option                         | Default       | Description                                                                                            |
| :----------------------------- | :------------ | :----------------------------------------------------------------------------------------------------- |
| `Interact`                     | `'auto'`      | `auto` uses ox\_target when it runs and the key prompt when it does not. `key` or `target` forces one. |
...
<Callout type="info">
  A few messages sent by the server (for example *A vehicle of yours was recovered and taken to the impound*) use the `ox_lib:notify` client event directly, so keep ox\_lib installed even when you use your own notifications.
</Callout>
