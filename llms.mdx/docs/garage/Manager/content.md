...
## Create a Garage [#create-a-garage]
...
<Callout type="info">
  Suggestions come from `qbx_garages` on Qbox and from `QBCore.Shared.Garages` on QBCore. On ESX and ox\_core you can type any storage name yourself.
</Callout>

## Impound Garages [#impound-garages]
...
* The fee is taken from the player's **bank**, and from their **cash** when the bank can't cover it. On ox\_core it is taken as the `money` item. The car only leaves once the fee is paid.
